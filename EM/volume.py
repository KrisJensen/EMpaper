...
class volume():
    ...
    def __init__(self, neurons=[], name='all', token=None):
    ...
    def add_neuron(self, neuron, name='default', subgroup=[], to_despike='None', rootnode='None', roottype='None',
        ...
        if not threshold == 'None':
            # we 'despike' the misaligned nodes for plotting
            despike.despike(neuron, despike, 8000, show=show)
        elif not to_despike == 'None':
            # this is pymaid's function
        ...
        # might want to do something if name already exists
        self.all_names[neuron.neuron_name] = neuron
    ...
    def init_subgroup(self, subgroup, neurons=[]):
        ...
        for neuron in neurons:
            self.add_to_subgroup(neuron, subgroup)  # add neurons to subgroup
        return self.groups[subgroup]

    def add_to_subgroup(self, neuron, subgroup):
        '''add neuron to subgroup'''
        if not subgroup in self.groups:
            # initialize if subgroup does not exist
            self.init_subgroup[subgroup]
    ...
    def subvolume(self, name, neurons=[]):
        ...
        self.subvolumes[name] = volume(skids, name=name, token=self.token)
    ...
    def add_to_subvolume(self, neuron, subvolume, name='default', subgroup=[], rootnode='None', roottype='None',):
        ...
        self.subvolumes[subvolume].add_neuron(
            neuron, name, subgroup, rootnode=rootnode, roottype=roottype)
    ...
    def split(self, neuron, cut1, cut2, rev=False, reverse=False, show=True, Type='None'):
        ...
        if Type == 'None':  # somewhat ad hoc to cut our specific neurons of interest
            ...
            N_EB = cut_neuron(cut_neuron(neuron, cut_node=cut1)[
                              1], cut_node=cut2)[0]
        ...
        if show:  # plot neuron as cut
            ...
            fig, ax = N_EB.plot2d(
                color='blue', method='2d', connectors=False, ax=ax)
            fig, ax = N_G.plot2d(color='green', method='2d',
    ...
    def get_json(self, neuron):
        ...
        neuron = requests.post('https://neuropil.janelia.org/tracing/fafb/v14/1/skeletons/connectivity',
                               data={'source_skeleton_ids[]': [skid],
                                     'boolean_op': 'OR', 'with_nodes': 'true'},
    ...
    def construct_cn_tables(self, check=False):
        ...
        # we make this a dict of dicts of connections cn_tables[pre][post] = connections
        cn_tables = dict()
        ...
        for pre in self.groups['all']:  # we need distances between all neurons
            ...
            # find all connections from the incoming neuron
            cn_table = pre_json['outgoing']

            for post in self.groups['all']:

                post = self.get_neuron(post)
                try:
                    # get the connections to post
                    connections = cn_table[post.skeleton_id]['links']
                except KeyError:
                    connections = []  # if there are no connections, the dict is empty
                except ConnectionError:
                    # we might have a network error and not want to crash
                ...
                if check:  # if we work with a cut neuron, we remove all entries that aren't in our fragment
                    ...
                    for cn in connections:
                        if cn[0] in pre.nodes.treenode_id.values:
                            ...
                            # add to list if in fragment
                            newconnections.append(cn)
                    connections = newconnections
                    print(connections)

                # add to dict
                cn_tables[pre.neuron_name][post.neuron_name] = connections
    ...
    def connection_number(self, pre, post, cn_table='None', from_connectors=False):
        ...
        if type(cn_table) == str:  # if we already have a cn table we can skip this
            if from_connectors:
                # get connection table for fragment
                cn_table = cn_table_from_connectors(pre)
            elif cn_table == 'stored':
                return len(self.cn_tables[pre][post])
            else:
                cn_table = pre.partners  # get connection table from intact neuron

        # we want to look at what this synapses onto
        downstream = cn_table[cn_table.relation == 'downstream']

        try:
            # this has number of synapses
            nSyn = downstream[downstream.skeleton_id == skid].total.values[0]
        except IndexError:
            nSyn = 0  # if there are no synapses
        except TypeError:
            try:
                # inconsistencies in pymaid means we sometimes need to use an int as key
                nSyn = downstream[downstream.skeleton_id ==
                                  int(skid)].total.values[0]
            except IndexError:
                nSyn = 0
    ...
    def allconnections(self, group1='all', group2='all', file='None', method='w', table='none',
        ...
        if group1 == 'all':
            group1 = self.groups['all']
        if group2 == 'all':
            group2 = self.groups['all']
        data = []
        tot = 0.0
        ...
        for pre in group1:  # consider each presynaptic neuron
            if not table == 'stored':
                if from_connectors:
                    ...
                    cn_table = pre.partners
            data.append([])
            for post in group2:
                if table == 'stored':
                    data[-1].append(len(self.cn_tables[pre.neuron_name]
                                        [post.neuron_name]))
                else:
                    # find connections to post
                    data[-1].append(self.connection_number(pre,
                                                           post, cn_table=cn_table))
        ...
        if not file == 'None':  # we write our result to a file
            with open('connectivity_matrices/'+file+'.txt', method) as f:
                f.write('{:<10}'.format(' '))
                for post in group2:
                    f.write('{:<10}'.format(post.neuron_name[:10])+' ')
                f.write('\n')
    ...
    def get_total_connections(self, neuron, relation, Type='local'):
        ...
        if Type == 'global':
            ...
            pickle.dump(cn_table, open('pickled_neurons/neuron_connections/' +
                                       self.name+'_'+relation+'_'+neuron.neuron_name+'_pickled', 'wb'))
        else:
            cn_table = pickle.load(open('pickled_neurons/neuron_connections/' +
                                        self.name+'_'+relation+'_'+neuron.neuron_name+'_pickled', 'rb'))
        # get all INPUTS/OUTPUTS for a neuron
        connections = cn_table[cn_table.relation == relation]

        if relation == 'upstream':
            self.full_cn_tables_input[neuron.neuron_name] = connections
        if relation == 'downstream':
            self.full_cn_tables_output[neuron.neuron_name] = connections
    ...
    def get_connection_distribution(self, relation='input'):
        ...
        for neuron in self.groups['all']:
            ...
            connections[name] = {}
            connections[name]['tot'] = self.get_total_connections(
                neuron, relation)
            for group in self.groups.keys():
                if group != 'all':
                    ...
                    for partner in self.groups[group]:
                        if relation == 'input':
                            cons += len(self.cn_tables[partner.neuron_name][name])
                        if relation == 'output':
                            cons += len(self.cn_tables[name]
                                        [partner.neuron_name])
    ...
    def plot_connection_distribution(self, relation='input', neurons_init='all',
        ...
        if neurons_init == 'all':
            neurons_init = self.groups['all']
        neurons = []
        # first we normalize the connection distribution per neuron
        if estimate:
            connections = copy.deepcopy(self.connectionEstimate[relation])
        else:
            connections = copy.deepcopy(self.connectionDistribution[relation])

        groups = ['PEN1', 'PEN2', 'EPG', 'PEG', 'D7']
        leg = ['PEN1 (8)', 'PEN2 (8)', 'EPG (10)', 'PEG (4)', 'D7 (7)']
        if estimate:
            leg = ['PEN1 (16)', 'PEN2 (16)', 'EPG (54)', 'PEG (16)', 'D7 (40)']
        ...
        for i, neuron in enumerate(neurons_init):
            if int(connections[neuron.neuron_name]['tot']) < 10:
                print('fewer than 10 connections for ' +
                      neuron.neuron_name+', not included in plot')
                ...
                if not (neuron.neuron_name in ['Ra', 'Rb', 'Rc', 'Rd', 'Re', 'Rf', 'Rg']):
                    ...
                    for key, number in connections[neuron.neuron_name].items():
                        ...
                        if Norm:
                            norm = number / \
                                connections[neuron.neuron_name]['tot']
                        else:
                            norm = number
                    ...
                    if include_unknown:
                        if Norm:
                            connections[neuron.neuron_name]['unknown'] = 1-cum
                        else:
                            connections[neuron.neuron_name]['unknown'] = connections[neuron.neuron_name]['tot']-cum
        N = len(neurons)
        ...
        for group in groups:
            if group != 'all':
                ...
                for neuron in neurons:
                    ...
                    f.write(group+' '+neuron.neuron_name+' ' +
                            str(connections[neuron.neuron_name][group])+'\n')
        f.close()
    ...
    def heatmap(self, data,  xticks=None, yticks=None, xlabel=None,
        ...
        # generate heatmap; could add more options
        ...
        plt.setp(ax.get_xticklabels(), rotation=-45, ha="right",
                 va='bottom', rotation_mode="anchor")  # make it look nice
    ...
    def plot2D(self, neurons='all', color=False, show=True, save='None'):
        ...
        if neurons == 'all':
            neurons = self.groups['all']
        else:
            neurons = [self.get_neuron(n) for n in neurons]  # neurons to plot
        if type(color) == list:
            color = color
        elif type(color) == str:
            color = [color for x in range(len(neurons))]
        else:
            # can specify colors of neurons
            color = ['red' for i in range(len(neurons))]
        # plot all of the neurons on the same set of axes
        for i, neuron in enumerate(neurons):
            try:
                fig, ax = neuron.plot2d(color=color[i], method='2d',
                                        connectors=False, ax=ax)
            except:
                fig, ax = neuron.plot2d(
                    color=color[i], method='2d', connectors=False)
...
class CatmaidApiTokenAuth(HTTPBasicAuth):
    ...
    def __init__(self, token, username=None, password=None):
        super(CatmaidApiTokenAuth, self).__init__('user', 'pw')
        self.token = token
