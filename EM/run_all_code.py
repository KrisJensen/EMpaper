...
# determines whether or not to include an explicit 'unknown' category
...
vol.subvolumes['PB'].plot_connection_distribution(
    relation='input', title='estimate_PB_input', estimate=True, Norm=Norm, include_unknown=include_unknown)
vol.subvolumes['PB'].plot_connection_distribution(
    relation='output', title='estimate_PB_output', estimate=True, Norm=Norm, include_unknown=include_unknown)
...
vol.subvolumes['NO'].plot_connection_distribution(
    relation='input', title='estimate_NO_input', estimate=True, Norm=Norm, include_unknown=include_unknown)
vol.subvolumes['NO'].plot_connection_distribution(
    relation='output', title='estimate_NO_output', estimate=True, Norm=Norm, include_unknown=include_unknown)
...
vol.subvolumes['EB'].plot_connection_distribution(
    relation='input', title='estimate_EB_input', estimate=True, Norm=Norm, include_unknown=include_unknown)
vol.subvolumes['EB'].plot_connection_distribution(
    relation='output', title='estimate_EB_output', estimate=True, Norm=Norm, include_unknown=include_unknown)
...
mod, consEB, consPB = write_combined_cons(
    continuous=True, fname='extrapolated/PB_EB_cont_extrapolated')
...
for neuron in ['PEN2-5R', 'PEN2-6Ra', 'PEN2-6Rb', 'PEN2-7R',
    ...
    # construct a tree
    t = treeNeuron(
        n, vol.neuron_inputs[n.neuron_name], vol.neuron_outputs[n.neuron_name])
