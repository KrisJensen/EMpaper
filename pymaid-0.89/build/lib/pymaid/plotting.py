...
def plot_adjacency(x, labels=True, remote_instance=None, **kwargs):
    ...
    ----------
    x :         pandas.DataFrame
                Adjacency matrix to plot. See :func:`pymaid.adjacency_matrix`.
...
def plot_tuning_curves(x, row_cluster=True, col_cluster=True, **kwargs):
    """ Plot tuning curves for a set of neurons.

    Parameters
    ----------
    x :             pandas.DataFrame
                    Adjacency matrix to plot. See :func:`pymaid.adjacency_matrix`.
                    For each row, a tuning curve is plotted.
    row_cluster :   bool | str, optional
                    If not ``False``, will cluster and reorder rows. Provide
                    clustering method as ``str`` (default='wards').
    col_cluster :   bool | str, optional
                    If not ``False``, will cluster and reorder columns. Provide
                    clustering method as ``str`` (default='wards').
    **kwargs
                    Will be passed to ``matplotlib.pyplot.bar``.
    ...
    Examples
    --------
    >>> adj = pymaid.adjacency_matrix('annotation:WTPN2017_excitatory_uPN_right',
    ...                               'annotation:AD1b2')
    >>> names = pymaid.get_names(adj.index.values)
    >>> all_gloms = set([re.search('glomerulus (.*?) ', v).group(1) for v in names.values()])
    >>> by_glom = {g: [s for s in names if g + ' ' in names[s]] for g in all_gloms}
    >>> adj_grouped = pymaid.group_matrix(adj, row_groups=by_glom)
    >>> fig, ax = pymaid.plot_tuning_curves(adj_grouped.T)
    >>> plt.show()
    """
    ...
    if row_cluster:
        if isinstance(row_cluster, str):
            method = row_cluster
        else:
            method = 'ward'

        # Calculate linkage
        link = scipy.cluster.hierarchy.linkage(x,
                                               method=method)
        # Get and apply order
        order = scipy.cluster.hierarchy.dendrogram(link,
                                                  orientation='left',
                                                  labels=x.index,
                                                  no_plot=True)['ivl']
        x = x.loc[order, :]
    ...
    if col_cluster:
        if isinstance(col_cluster, str):
            method = col_cluster
        else:
            method = 'ward'

        # Calculate linkage
        link = scipy.cluster.hierarchy.linkage(x.T,
                                               method=method)
        # Get and apply order
        order = scipy.cluster.hierarchy.dendrogram(link,
                                                  orientation='left',
    ...
    fig, axes = plt.subplots(x.shape[0], 1,
    ...
    if 'color' in kwargs:
        ...
        color = sns.color_palette('muted', x.shape[1])

    xpos = np.arange(0, x.shape[1]) + .5
    for v, l, ax in zip(x.values, x.index, axes):
        ax.bar(xpos, v, color=color, **kwargs)
        ax.set_ylabel(l, rotation=90, ha='center', va='center', fontsize=9)
